...
if [ -e "${OFFLINEIMAP_CONF_FILE}" ]; then
    ...
    # svn mails
    ${NOTMUCH_BIN} tag +svn -unread -- tag:new and from:svnmailer@cosmo-platform.org and not "/csmSDK/trunk/csmCore"
    ${NOTMUCH_BIN} tag +svn -unread -- tag:new and from:svnmailer@cosmo-platform.org and not "in /csmSDK/trunk: csmCore/"
